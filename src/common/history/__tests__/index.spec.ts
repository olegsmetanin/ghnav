...
describe('history', () => {
  it('works', async () => {
    ...
    expect(history.get()).toEqual([{ pathname: 'issue', query: { number: '5638', owner: 'zeit', repo: 'next.js' } }])
    ...
    expect(hs.history).toEqual([{ pathname: 'issue', query: { owner: 'zeit', repo: 'next.js', number: '5638' } }])
  })
})
